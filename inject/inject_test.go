...
import (
	...
	injecttesta "github.com/tommynurwantoro/gontainer/inject/injecttesta"
	injecttestb "github.com/tommynurwantoro/gontainer/inject/injecttestb"
)

// sameElements checks if two slices contain the same elements regardless of order.
func sameElements(t *testing.T, actual, expected []string) {
	t.Helper()
	if len(actual) != len(expected) {
		t.Fatalf("length mismatch: expected %d elements, got %d. Expected: %v, Actual: %v", len(expected), len(actual), expected, actual)
	}
	...
	if !reflect.DeepEqual(actualMap, expectedMap) {
		t.Fatalf("elements mismatch: expected %v, got %v", expected, actual)
	}
}
...
func TestProvideWithFields(t *testing.T) {
	...
	expected := "fields were specified on object *inject_test.TypeAnswerStruct when it was provided"
...
func TestProvideTwoOfTheSame(t *testing.T) {
	...
	expectedPrefix := "provided two unnamed instances of type *"
	if !strings.HasPrefix(err.Error(), expectedPrefix) || !strings.Contains(err.Error(), "TypeAnswerStruct") {
		t.Fatalf("expected error message containing %q and TypeAnswerStruct, got: %s", expectedPrefix, err.Error())
	}
}
